...
<spring:url value="/CompanyDtl/CompanyDtlList"     var="urlHome" />

<spring:url value="/CompanyDtl/CompanyDtlAdd"      var="addURL" />

<nav class="navbar navbar-light" style="background-color: #e3f2fd;">
  <a class="navbar-brand">Company Details</a>
  <form class="form-inline">   
     <a class="navbar-brand" href="${addURL}"><p class="text-primary">ADD Company Details</p></a>
  </form>
</nav>
           ...
           <div id="indentItemListDiv" style="width:100%;">
				...
				<thead>
					<tr>	
						...
						<th colspan='1'>Company RefNo</th>
						<th colspan='1'>Company Email</th>
						<th colspan='1'>Date Of Registration</th>
						<th colspan='1'>Action</th> 
					</tr>
				</thead>
	            <tbody>
				<c:forEach var="ComDtl" items="${unitmasterBean}">
				<input id="strHiddenId" name="strHiddenId" type="hidden" value="${ComDtl.strHiddenId}"/>
					<tr>		
					    <td>${ComDtl.strHiddenId}</td> 			
						<td>${ComDtl.strCompanyName}</td>
						<td>${ComDtl.strComRefNo}</td>	
						<td>${ComDtl.strCompanyEmailId}</td>	
						<td>${ComDtl.strDateOfReg}</td>					
						<td>
							<spring:url value="/CompanyDtl/${ComDtl.strHiddenId}/sourceUnitAdd"  var="addSourceUnitUrl" />
							<spring:url value="/CompanyDtl/${ComDtl.strHiddenId}/companyDtlsModify"  var="updateUrl" />
	                        <spring:url value="/CompanyDtl/${ComDtl.strHiddenId}/companyDtlsDelete"  var="deleteURL" />
	                        <spring:url value="/CompanyDtl/${ComDtl.strHiddenId}/companyDtlsView"    var="viewURL" />  
	                        	                        
	                      	<button class="btn btn-success" onclick="location.href='${addSourceUnitUrl}'">Add Source Unit</button>
