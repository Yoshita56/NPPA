...
<spring:url value="/UnitMst/UnitMasterList"     var="urlHome" />

<spring:url value="/UnitMst/UnitMasterAdd"      var="addURL" />

<nav class="navbar navbar-light" style="background-color: #e3f2fd;">
  <a class="navbar-brand">Unit/Package Type Master</a>
  <form class="form-inline">   
     <a class="navbar-brand" href="${addURL}"><p class="text-primary">ADD Unit/Package Type</p></a>
  </form>
</nav>
           ...
           <div id="indentItemListDiv" style="width:100%;">
				...
				<thead>
					<tr>	
						...
						<th colspan='1'>Converted Value</th>
						<th colspan='1'>Effective From</th>
						<th colspan='1'>Action</th> 
					</tr>
				</thead>
	            <tbody>
				<c:forEach var="UnitMSt" items="${unitmasterBean}">
				<input id="strHiddenId" name="strHiddenId" type="hidden" value="${UnitMSt.strHiddenId}"/>
					<tr>		
					    <td>${UnitMSt.strHiddenId}</td> 			
						<td>${UnitMSt.strUnitSize}</td>
						<td>${UnitMSt.strBaseUnitName}</td>	
						<td>${UnitMSt.strConvertedValue}</td>	
						<td>${UnitMSt.strEffectiveFrom}</td>					
						<td>
					
							<spring:url value="/UnitMst/${UnitMSt.strHiddenId}/unitmastermodify"  var="updateUrl" />
	                        <spring:url value="/UnitMst/${UnitMSt.strHiddenId}/unitmasterdelete"  var="deleteURL" />
	                        <spring:url value="/UnitMst/${UnitMSt.strHiddenId}/unitmasterview"    var="viewURL" />  
