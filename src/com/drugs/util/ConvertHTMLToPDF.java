...
public class ConvertHTMLToPDF  {
	...
	/**
	 * Instantiates a new convert html to pdf.
	 */
	...
	/**
	 * * Notes 1. Let 80 Spaces means One Line And 2. Let 9 Spaces Means One
	 * Column
	 * 
	...
	private boolean addHTMLElementToStr(TableTag node) {
		...
		if (node.getAttribute("id") != null) {
			...
			if (nl != null)
				for (SimpleNodeIterator ite = nl.elements(); ite.hasMoreNodes();)
					haveContent = addHTMLElementToStr((Node) ite.nextNode())
							|| haveContent;
			if (!node.getAttribute("id").startsWith("tblTemplate#")
					&& !haveContent)
				this.targetContent += "         ";
	...
	private boolean addHTMLElementToStr(TextNode node) {
		...
		if (tdImmediate.getAttribute("align") != null) {
			...
			if (tdImmediate.getAttribute("align").toLowerCase().equals("left"))
				;
			else if (tdImmediate.getAttribute("align").toLowerCase()
					.equals("right"))
				finalValue = space + value;
			else if (tdImmediate.getAttribute("align").toLowerCase()
					.equals("center"))
				finalValue = space.substring(0, space.length() / 2) + value
	...
	private boolean addHTMLElementToStr(InputTag node) {
		...
		if (node.getAttribute("type").equals("text"))
			...
			if (node.getAttribute("checked") != null
					&& node.getAttribute("checked").equals("true"))
				value = node.getAttribute("value");
				...
				for (SimpleNodeIterator ite = nl.elements(); ite.hasMoreNodes();) {
					...
					if (childNode instanceof OptionTag) {
						...
						if (optNode.getAttribute("selected") != null
								&& optNode.getAttribute("selected").equals(
										"true")) {
	...
	/**
	 * * Notes 1. Adding Empty Paragraphs followed by Chunks in Array List 2.
	 * Nothing will addded to Paragraph 3. Only in Chunks
	 * 
	 * @return true, if successful
	 */
			...
			for (int i = 0; i < this.alDocElements.size(); i++) {
				...
				if (ele instanceof Paragraph) {
					if (para != null)
						this.doc.add(para);
					para = (Paragraph) ele;
				} else if (ele instanceof Chunk
						&& !((Chunk) ele).getContent().equals(""))
					para.add(ele);
			}
			if (para != null)
				this.doc.add(para);
	...
	private boolean addHTMLToDocElement(TableTag node) {
		...
		if (this.alDocElements.size() > 0
				&& this.alDocElements.get(this.alDocElements.size() - 1) != null)
			lastDocEleInList = (Element) this.alDocElements
					.get(this.alDocElements.size() - 1);
		...
		if (node.getAttribute("id") != null) {
			...
			if (nl != null)
				for (SimpleNodeIterator ite = nl.elements(); ite.hasMoreNodes();)
					haveContent = addHTMLToDocElement((Node) ite.nextNode())
							|| haveContent;
			if (!node.getAttribute("id").startsWith("tblTemplate#")
					&& !haveContent)
	...
	private boolean addHTMLToDocElement(TextNode node) {
		...
		SetAlignment(node);
		Chunk chk = null;
		if (lastDocEleInList instanceof Chunk
				&& ((Chunk) lastDocEleInList).getContent().equals(""))
	...
	private boolean SetAlignment(Node node) {
		...
		if (tdImmediate.getAttribute("align") != null) {
			...
			if (col == 9) {
				...
				if (this.alDocElements.size() > 0
						&& this.alDocElements
								.get(this.alDocElements.size() - 1) != null)
					tempEle = (Element) this.alDocElements
							.get(this.alDocElements.size() - 1);
				if (tempEle != null) {
					int i = this.alDocElements.size() - 2;
					while (i >= 0 && !(tempEle instanceof Paragraph))
						tempEle = (Element) this.alDocElements.get(i);
					if (tempEle instanceof Paragraph)
						para = (Paragraph) tempEle;
				}
				if (para == null) {
					para = aNewParagraph();
					this.alDocElements.add(para);
				}
				if (tdImmediate.getAttribute("align").toLowerCase()
						.equals("left"))
	...
	private boolean addHTMLToDocElement(InputTag node) {
		...
		if (!value.equals("")) {
			...
			if (font.getStyle() != -1)
				font.setStyle(font.getStyle() | Font.BOLD);
			else
	...
	private boolean addHTMLToDocElement(Tag node) {
		...
		if (node.getTagName().toLowerCase().equals("b")) {
			...
			Font font = chk.getFont();
			Color color = new Color(Integer.parseInt(node.getAttribute("color")
					.replace("#", ""), 16));
			font.setColor(color);
		}
		return false;
	...
	/**
	 * * Notes 1. Replacing PARMATER Tags to Values if exists Otherwise to do
	 * Empty 2. In Consent Remove Print TableRow 3. Only in Chunks
	 * 
	 * @param map
	 *            the map
	 * @return true, if successful
	...
	private void setTagToValue(TableColumn node) {
		...
		if (node.getAttribute("id") != null
				&& node.getAttribute("id").equals("tdPrintConsent")) {
	...
	private void setTagToValue(InputTag node) {
		...
		if (node.getAttribute("name") != null
				&& node.getAttribute("name").startsWith("PARAMETER")) {
