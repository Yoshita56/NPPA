...
public class Printer {
	...
	/** The Constant TEMP_DIR_NAME. */
	private static final String TEMP_DIR_NAME = System.getProperties()
			.getProperty("java.io.tmpdir");

	/** The Constant OS_NAME. */
	private static final String OS_NAME = System.getProperties().getProperty(
			"os.name");
	...
	/**
	 * Gets the converted host name.
	 * 
	 * @param dottedHostName
	 *            the dotted host name
	 * @return the converted host name
	 */
	...
	public synchronized boolean createTempFile(String contents, String hostName) {
		...
		// String dateTime = new
		// registration.FuncLib().getSysdate("YYYYMonDDHH12_MIAM_");

		if (OS_NAME.startsWith("Linux"))
			fileName = "/root/nike/regCard_"
					+ new Printer().getConvertedHostName(hostName) + ".txt";
		// fileName="/root/nike/regCard_"+new
		// Printer().getConvertedHostName(hostName)+dateTime+".txt";
		else
			fileName += "regCard_"
					+ new Printer().getConvertedHostName(hostName) + ".dat";
	...
	public static boolean printSlip(String hostName) {
		...
		if (OS_NAME.startsWith("Linux")) {
			...
			if (OS_NAME.startsWith("Windows"))
				retValue = windowsPrinting("\\\\" + hostName + "\\printername",
						hostName); // For windows based printing
			else {
				System.out
						.println("Unable to detect the Operating System. Pritnting Aborted");
				retValue = false;
			}
		}
		return retValue;
	...
	public static boolean linuxPrinting(String hostName) {
		...
		// registration.FuncLib().getSysdate("YYYYMonDDHH12_MIAM_");
		boolean retValue = false;
		// String fileName = "/root/regCard_"+new
		// BeanRegPrinterSupport().getConvertedHostName(hostName)+".dat";
		// String fileName ="/root/nike/regCard_"+new
		// Printer().getConvertedHostName(hostName)+dateTime+".txt";
		String fileName = "/root/nike/regCard_"
				+ new Printer().getConvertedHostName(hostName) + ".txt";
		String printCommand = "echo \"print\" " + fileName + "|smbclient //"
				+ hostName + "/printername -U Administrator%";
	...
	public static boolean windowsPrinting(String printerPath, String hostName) {
		...
		String fileName = TEMP_DIR_NAME + "regCard_"
				+ new Printer().getConvertedHostName(hostName) + ".dat";
		boolean retValue = false;
