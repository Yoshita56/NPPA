...
public final class HISTicketRegistry implements TicketRegistry
{
	...
	public void addTicket(final Ticket ticket,HttpSession objSession)
	{
		...
		{
			this.userTicketCache.put(ticket.getAuthentication().getVarUserId(), ticket);
			this.userIPTicketCache.put(ticket.getAuthentication().getVarUserId()+"#"+ticket.getAuthentication().getVoUser().getVarIPAddress()+"#"+objSession.getId(), ticket);
		}
	}
	...
	public boolean deleteTicket(final String ticketId)
	{
		...
		if(ticket != null)
		{
			...
			else
			{
				...
				this.userIPTicketCache.remove(ticket.getAuthentication().getVarUserId()+"#"+ticket.getAuthentication().getVoUser().getVarIPAddress()+"#"+ ticket.getAuthentication().getVarUserLoginSessionId());
			}
